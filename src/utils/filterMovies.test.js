...
describe("filterMovies", () => {
  it("searches titles and genres without changing the source list", () => {
    const source = [...movies];
    expect(filterMovies(movies, { search: "science fiction" })).toEqual([movies[0]]);
  ...
  it("filters by genre and supports each sort mode", () => {
    ...
    expect(filterMovies(movies, { sort: "newest" }).map(movie => movie.id)).toEqual([1, 3, 2]);
    expect(filterMovies(movies, { sort: "title" }).map(movie => movie.id)).toEqual([2, 3, 1]);
  });
});
