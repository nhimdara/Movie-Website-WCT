...
export default function SearchBar({ open, onClose }) {
  ...
  return (
    <div className={`search-panel${open ? " open" : ""}`} aria-hidden={!open}>
      <form className="container global-search-form" action="/movies">
        <label className="sr-only" htmlFor="global-search">
          {t("searchMovies", "Search movies")}
        </label>
        <input
          id="global-search"
          name="search"
          type="search"
          placeholder={`${t("searchMovies", "Search by title, genre or year")}…`}
          value={query}
        ...
        <button className="btn btn-primary">
          {t("seeAllResults", "See all results")} <span>→</span>
        </button>
      </form>
      <div className="container search-suggestions">
        <div className="search-suggestion-label">
          <span>{query ? "Matching titles" : "Popular now"}</span>
        ...
        {results.length ? (
          ...
          <p className="search-no-results">No movies match “{query}”.</p>
        )}
      </div>
    </div>
  );
