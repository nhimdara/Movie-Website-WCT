...
const Icon = ({ name, size = 19 }) => {
  const paths = {
    ...
    star: (
      <path d="m12 3 2.8 5.67 6.2.9-4.5 4.38 1.06 6.18L12 17.2l-5.56 2.93 1.06-6.18L3 9.57l6.2-.9Z" />
    ),
    eye: (
      <>
...
function TrendChart({ range }) {
  ...
  const points = values
    .map((value, index) => `${20 + index * 46},${190 - value * 1.5}`)
    .join(" ");
  ...
  return (
    <div className="dash-chart">
      ...
      <svg
        viewBox="0 0 546 210"
        role="img"
        aria-label="Views have increased over the selected period"
      >
        <defs>
...
const sectionCopy = {
  ...
  Reviews: [
    "Review moderation",
    "Manage ratings and feedback from your community.",
  ],
  Inbox: [
    "Contact inbox",
    "Read and manage messages submitted through the website.",
  ],
  Website: [
    "Website content",
    "Control the information visitors see across your public website.",
  ],
  Settings: [
    "Studio settings",
    "Control workspace preferences and notifications.",
  ],
};
      ...
      <>
        ...
        <div className="library-stats">
          ...
          <article>
            <span>Drafts</span>
            <strong>
              {movies.filter((movie) => movie.status === "draft").length}
            </strong>
            <small>Only visible to admins</small>
          </article>
          <article>
            <span>Playable movies</span>
            <strong>{movies.filter((movie) => movie.videoUrl).length}</strong>
            <small>Full video available</small>
          </article>
        </div>
        <section className="panel library-panel">
          ...
          {filteredMovies.length ? (
            filteredMovies.map((movie) => (
              <div className="library-row" key={movie.id}>
                ...
                >
                  <i /> {movie.status === "published" ? "Published" : "Draft"}
                </span>
            ...
            <div className="dashboard-empty">No movies match “{query}”.</div>
          )}
        </section>
      </>
    );
    ...
    return (
      <>
        ...
        <div className="audience-layout">
          ...
          <section className="panel audience-locations">
            <div className="panel-heading">
              <div>
                <h2>Top locations</h2>
                <p>Viewers by region</p>
              </div>
            </div>
            {[
              ["United States", 38],
    ...
    return (
      <>
        ...
        <div className="analytics-cards">
          {[
            ["Completion rate", "72.4%", "+4.2%"],
            ["Returning viewers", "41.8%", "+7.1%"],
            ["Avg. session", "48m 12s", "+2.9%"],
          ].map(([label, value, delta]) => (
            <article className="panel" key={label}>
              <span>{label}</span>
              <strong>{value}</strong>
              <small>{delta} this period</small>
            </article>
          ))}
        </div>
    ...
    return (
      <>
        ...
        <section className="review-grid">
          {reviews.map((review) => (
            <article
              className={`panel review-card ${review.status !== "pending" ? "resolved" : ""}`}
              key={review.id}
            >
              ...
              {review.status === "pending" ? (
                <div className="review-actions">
                  ...
                  <button onClick={() => moderateReview(review.id, "hidden")}>
                    Hide
                  </button>
                </div>
              ) : (
        ...
        <section className="inbox-list">
          {messages.length ? (
            messages.map((message) => (
              <article
                className={`panel inbox-message ${message.status === "resolved" ? "resolved" : ""}`}
                key={message.id}
              >
  ...
  return (
    <>
      ...
      <section className="settings-layout">
        <nav className="panel settings-nav" aria-label="Settings section">
          <span className="active">General preferences</span>
          <small>Notification and publishing behavior is managed here.</small>
        </nav>
        <form
        ...
        >
          <div className="settings-heading">
            ...
            <p>Customize how your studio workspace behaves.</p>
          </div>
          ...
          <div className="settings-switches">
            {[
              ...
              [
                ...
                "Get notified when viewers leave feedback.",
                "reviews",
              ],
              [
                "Auto-publish approved titles",
                "Publish immediately after editorial approval.",
                "publish",
              ],
            ].map(([title, copy, key]) => (
              <label key={key}>
          ...
          <div className="settings-save">
            <span className={saved ? "show" : ""}>✓ Changes saved</span>
...
function WebsiteEditor({ movies, settings, onSave, onReset, saved }) {
  ...
  const visibility = [
    ...
    [
      "Continue watching",
      "Show viewing history to returning visitors.",
    ...
    [
      "Genre collections",
      "Show Science Fiction, Drama and Thriller rows.",
      "showGenreRows",
    ],
    ...
    <>
      ...
      <form className="website-editor" onSubmit={submit}>
        <section className="panel website-settings-card">
          <div className="settings-heading">
            <h2>Brand and homepage</h2>
            <p>These changes appear immediately after you save.</p>
          </div>
          <div className="website-logo-control">
            <div className="website-logo-preview">
              {logoPreview ? (
            ...
            <div>
              <b>Website logo</b>
              <small>
                Upload a square or horizontal PNG, JPG, WebP or SVG. Maximum 750
                KB.
              </small>
              <div>
                <label className="website-logo-upload">
                  Choose logo
                  <input
                    type="file"
          ...
          <div className="website-field-grid">
            ...
            <label>
              <span>Hero editor note</span>
              <input name="heroNote" defaultValue={settings.heroNote} />
            </label>
            <label>
              <span>Featured hero movie</span>
              <select
                name="featuredMovieId"
                defaultValue={settings.featuredMovieId}
              >
                <option value="">Automatic — first published movie</option>
                {movies.map((movie) => (
            ...
            <label>
              <span>Top titles heading</span>
              <input name="topRowTitle" defaultValue={settings.topRowTitle} />
            </label>
            <label>
              <span>Trending heading</span>
              <input
                name="trendingTitle"
                defaultValue={settings.trendingTitle}
              />
            </label>
            <label>
              <span>New releases heading</span>
              <input
                name="newReleasesTitle"
                defaultValue={settings.newReleasesTitle}
              />
            </label>
            <label>
              ...
              <input
                ...
                placeholder="Optional message shown above the navigation"
              />
            </label>
          </div>
        </section>
        ...
        <section className="panel website-settings-card">
          <div className="settings-heading">
            <h2>Public page information</h2>
            <p>Manage user-facing copy without editing source files.</p>
          </div>
          <div className="website-field-grid">
            <label>
              <span>Contact email</span>
            ...
            <label>
              <span>About page heading</span>
              <input name="aboutHeading" defaultValue={settings.aboutHeading} />
            </label>
            <label>
              <span>Contact page heading</span>
              <input
                name="contactHeading"
            ...
            <label className="wide">
              <span>About page description</span>
              <textarea
                name="aboutCopy"
                rows="3"
            ...
            <label className="wide">
              <span>Contact page description</span>
              <textarea
                name="contactCopy"
                rows="3"
            ...
            <label className="wide">
              ...
              <textarea
                name="newsletterCopy"
                rows="2"
                defaultValue={settings.newsletterCopy}
              />
            </label>
          </div>
        </section>
        ...
        <section className="panel website-settings-card">
          <div className="settings-heading">
            ...
            <p>Choose which content visitors can see.</p>
          </div>
          <div className="website-switches">
            {visibility.map(([title, copy, key]) => (
              <label key={key}>
                <span>
                  <b>{title}</b>
                  <small>{copy}</small>
        ...
        <footer className="website-editor-actions">
          <button type="button" className="website-reset" onClick={onReset}>
            Restore defaults
          </button>
...
export default function AdminDashboard() {
  ...
  const exportReport = () => {
    const rows = [
      ...
      ...movies.map((m) => [m.title, m.genre, m.year, m.rating]),
    ];
    const csv = rows
  ...
  const resetWebsite = () => {
    if (
      !window.confirm(
        "Restore all public website text and section visibility to the defaults?",
      )
    )
      return;
    resetSiteSettings();
  ...
  const removeAllMovies = () => {
    if (
      window.confirm(
        "Delete every movie in the catalogue? Favourites and watchlists will also be cleared.",
      )
    )
      deleteAllMovies();
  };
  ...
  const restoreMovies = () => {
    if (
      window.confirm(
        "Replace the current catalogue with the original seed data?",
      )
    )
      restoreSeedMovies();
  };
  ...
  const exportData = () => {
    ...
    link.href = URL.createObjectURL(
      new Blob([JSON.stringify(movies, null, 2)], { type: "application/json" }),
    );
  ...
  return (
    <div className="admin-shell">
      <button
        className={`dash-backdrop ${menuOpen ? "show" : ""}`}
        aria-label="Close menu"
        onClick={() => setMenuOpen(false)}
      />
      <aside className={`admin-sidebar ${menuOpen ? "open" : ""}`}>
        <a className="admin-brand" href="/">
          <span className="admin-brand-mark has-logo">
            ...
            <div className="admin-nav-group" key={group.label}>
              ...
              {group.items.map(([icon, label]) => (
                ...
                >
                  ...
                  {label === "Inbox" &&
                    messages.some((message) => message.status === "new") && (
                      <b>
                        {
        ...
        <div className="admin-profile">
          <span
            className={`admin-avatar${session?.avatar ? " has-avatar" : ""}`}
          >
            {session?.avatar ? (
        ...
        <header className="admin-topbar">
          ...
          <div className="admin-top-actions">
            ...
            >
              <Icon name="bell" />
              {(messages.length > 0 ||
                reviews.some((review) => review.status === "pending")) && <i />}
            </button>
          ...
          {active === "Overview" ? (
            <>
              <section className="admin-welcome">
                <div>
                  <p className="admin-kicker">Saturday, July 25</p>
              ...
              <section className="dashboard-grid">
                <article className="panel performance-panel">
                  <div className="panel-heading">
                    <div>
                      <h2>Viewing performance</h2>
                      <p>Unique views across all published titles</p>
                    </div>
                    <div className="range-tabs">
                      {["7d", "30d", "90d"].map((item) => (
                        <button
                ...
                <article className="panel genre-panel">
                  <div className="panel-heading">
                    <div>
                      ...
                      <p>Titles in your catalogue</p>
                    </div>
                    <button className="more-button">•••</button>
                  </div>
                  <div className="genre-total">
                  ...
                  <div className="genre-bar">
                    {genreStats.map(({ genre, count }, index) => (
                      <i
                        key={genre}
                        style={{
                          width: `${(count / Math.max(movies.length, 1)) * 100}%`,
                        }}
                        className={`genre-${index}`}
                      />
                    ))}
                  </div>
                  <div className="genre-list">
                    {genreStats.map(({ genre, count }, index) => (
                      <div key={genre}>
                        <i className={`genre-${index}`} />
                        <span>{genre}</span>
                        <b>{count}</b>
                        <small>
                          {Math.round(
                            (count / Math.max(movies.length, 1)) * 100,
                          )}
                          %
                        </small>
                      </div>
                ...
                <article className="panel titles-panel">
                  <div className="panel-heading">
                    <div>
                      <h2>Top performing titles</h2>
                      <p>Ranked by audience score and engagement</p>
                    </div>
                    <a href="/movies">
                      View all <Icon name="chevron" size={15} />
                    </a>
                  </div>
                  <div className="title-table-head">
                    <span>Title</span>
                    <span>Views</span>
                    <span>Rating</span>
                    <span>Trend</span>
                      ...
                      rankedMovies.map((movie, index) => (
                        ...
                        >
                          ...
                          <span className="movie-views">
                            {["248K", "192K", "174K", "143K", "118K"][index]}
                          </span>
                          <span className="movie-rating">
                            <Icon name="star" size={14} />
                            {movie.rating}
                          </span>
                          <span className="movie-trend">
                            +{[24, 18, 13, 9, 7][index]}%
                          </span>
                  ...
                  <div className="activity-list">
                    {[
                      ...
                      [
                        "message",
                        "Neon Abyss",
                        "New critic review added",
                        "4 hrs ago",
                        "violet",
                      ],
                    ].map(([icon, title, copy, time, color]) => (
            ...
            <WorkspacePage
              ...
              onDeleteAll={removeAllMovies}
              onRestore={restoreMovies}
              onImport={importData}
              onExport={exportData}
            />
          )}
...
function MovieEditor({ movie, onSave, onClose }) {
  ...
  const [fileTrailer, setFileTrailer] = useState(
    movie?.trailerUrl?.startsWith("data:video/") ? movie.trailerUrl : "",
  );
  const [trailerFileName, setTrailerFileName] = useState(
    movie?.trailerUrl?.startsWith("data:video/") ? "Uploaded trailer" : "",
  );
  ...
  const handleTrailerFile = (event) => {
    ...
    if (file.size > 2_000_000) {
      window.alert(
        "Uploaded trailers must be smaller than 2 MB. For larger videos, paste a YouTube, Vimeo or hosted video URL.",
      );
      event.target.value = "";
      return;
    }
  ...
  return (
    <div
      className="movie-editor-backdrop"
      role="dialog"
      aria-modal="true"
      aria-label={movie ? "Edit movie" : "Add movie"}
    >
      <form className="movie-editor" onSubmit={submit}>
        <header>
          <div>
            <span>
              {movie ? "Edit catalogue title" : "New catalogue title"}
            </span>
            <h2>{movie ? movie.title : "Add a movie"}</h2>
          </div>
        ...
        <div className="movie-editor-body">
          <aside>
            <img
              ...
              alt="Poster preview"
              onError={(event) => {
                event.currentTarget.src = "/images/movies/movie-1.webp";
              }}
            />
          ...
          <div className="movie-fields">
            ...
            <label>
              <span>Genre *</span>
              <select name="genre" defaultValue={movie?.genre || genres[0]}>
                {genres.map((genre) => (
            ...
            <div className="wide trailer-source-field">
              <label>
                ...
                <input
                  name="trailerUrl"
                  type="url"
                  placeholder="YouTube, Vimeo, .mp4, .webm or .ogg URL"
                  defaultValue={
                    movie?.trailerUrl?.startsWith("data:video/")
                      ? ""
                      : movie?.trailerUrl || ""
                  }
                  onChange={() => {
              ...
              <div className="trailer-source-divider">
                <span>or</span>
              </div>
              <label className="trailer-upload">
                <b>Upload trailer file</b>
                <small>
                  {trailerFileName || "MP4, WebM or OGG · max 2 MB"}
                </small>
                <input
                  type="file"
            ...
            <label className="wide">
              <span>Full movie video URL</span>
              <input
                name="videoUrl"
                type="url"
                placeholder="YouTube, Vimeo, hosted .mp4, .webm or .ogg URL"
                defaultValue={movie?.videoUrl || ""}
              />
              <small className="field-help">
                Use a hosted URL for full movies. Large movie files cannot be
            ...
            <label>
              <span>Cast</span>
              <input name="cast" defaultValue={movie?.cast || ""} />
            </label>
            <label className="wide">
              <span>Publishing status</span>
              <select name="status" defaultValue={movie?.status || "published"}>
        ...
        <footer>
          ...
          <button className="save-movie-button">
