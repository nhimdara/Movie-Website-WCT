...
export default function MovieBanner() {
  ...
  >
    ...
    <div key={`content-${movie.id}`} className="hero-content hero-slide-content" aria-live="polite">
      ...
      <h1>{leadWords && <>{leadWords}<br /></>}<em>{accentWord}</em></h1>
      <p>{movie.description}</p>
      <div className="hero-editor-note"><span>Editor&apos;s selection</span><i/><small>{siteSettings.heroNote}</small></div>
      <div className="hero-meta"><span>★ {movie.rating}</span><span>{movie.genre}</span><span>{movie.duration}</span></div>
      <div className="button-row">
        <button className="btn btn-primary" onClick={() => movie.videoUrl ? playMovie(movie) : playTrailer(movie)}><PlayIcon /> {movie.videoUrl ? "Watch movie" : "Watch trailer"}</button>
        <button className={`btn btn-ghost${saved ? " is-added" : ""}`} onClick={() => toggleWatchlist(movie.id)}><PlusIcon checked={saved} /> My watchlist</button>
        <a className="hero-details-link" href={`/movie?id=${movie.id}`}>Film details <span>↗</span></a>
      </div>
    </div>
    ...
    {slides.length > 1 && <button className="hero-next-card" onClick={goNext} aria-label={`Next movie: ${nextMovie.title}`}>
      ...
      <span><small>Up next</small><b>{nextMovie.title}</b><i>{nextMovie.genre} · {nextMovie.year}</i></span>
      <strong>→</strong>
    </button>}
    <a className="hero-scroll-cue" href="#trending"><i/><span>Scroll to discover</span></a>
